...
func main() {
	...
	var matches []*coords
	var distances []int
	wire1 = wireCoords(wire1, l1)
	wire2 = wireCoords(wire2, l2)
	matches = findMatches(wire1, wire2)
	distances = manhattanDist(matches)
	fmt.Println(distances[1])
}

func wireCoords(wire []*coords, turns string) []*coords {
	var xOffset int
	var yOffset int
	wire = append(wire, newCoords(0, 0))
	wireTurns := strings.Split(turns, ",")
	for _, turn := range wireTurns {
		direction := turn[0:1]
		units, _ := strconv.Atoi(turn[1:])
		switch direction {
		case "U":
			xOffset, yOffset = 0, 1
		case "D":
		...
		for c := 1; c <= units; c++ {
			wire = append(wire, newCoords(wire[len(wire)-1].x+xOffset, wire[len(wire)-1].y+yOffset))
		}
	}
	return wire
}

func findMatches(wire1 []*coords, wire2 []*coords) []*coords {
	var output []*coords
	for _, c1 := range wire1 {
		for _, c2 := range wire2 {
			if *c1 == *c2 {
				output = append(output, c1)
			}
		}
	}
	return output
...
func manhattanDist(matches []*coords) []int {
	// returns sorted shortest to longest distances
	var distances []int
	for _, coord := range matches {
		distances = append(distances, absInt(coord.x)+absInt(coord.y))
	}
	sort.Ints(distances)
	return distances
